...
const WeatherSetting = ({cityName, handleCurrentCityChange, handleCurrentPageChange}) => {
  ...
  // const [locationName, setLocationName] = useState('臺北市'); // location 為 window.location 物件
  const inputLocationRef = useRef(null);

  const handleChange = (e) => {
    // console.log(e.target.value);
    setLocationName(e.target.value);
    localStorage.setItem('cityName', locationName); // Chrome/Application/Storage/Local Storage
  }

  const handleSave = (e) => {
    // console.log('locationName',locationName);
    // console.log('value',inputLocationRef.current.value);
    handleCurrentCityChange(locationName);    // 傳 props 給 App.js
    handleCurrentPageChange('WeatherCard');   // 傳 props 給 App.js
  }
  return (
